...
def test_package_output(tmpdir, capsys, install_mockery, mock_fetch):
    ...
    spec = spack.concretize.concretize_one(Spec("printing-package"))
    pkg = spec.package
...
def test_install_env_variables(mock_packages, mock_archive, mock_fetch, install_mockery):
    spec = spack.concretize.concretize_one(Spec("libdwarf"))
    install("libdwarf")
...
def test_install_commit(mock_git_version_info, install_mockery, mock_packages, monkeypatch):
    ...
    spec = spack.concretize.concretize_one(Spec(f"git-test-commit@{commits[-1]}"))
...
def test_install_overwrite_multiple(mock_packages, mock_archive, mock_fetch, install_mockery):
    # Try to install a spec and then to reinstall it.
    libdwarf = spack.concretize.concretize_one(Spec("libdwarf"))

    install("libdwarf")

    cmake = spack.concretize.concretize_one(Spec("cmake"))
...
def test_extra_files_are_archived(mock_packages, mock_archive, mock_fetch, install_mockery):
    s = spack.concretize.concretize_one(Spec("archive-files"))
...
def test_cdash_install_from_spec_json(
    ...
    with capfd.disabled(), tmpdir.as_cwd():
        ...
        pkg_spec = spack.concretize.concretize_one(Spec("pkg-a"))
...
def test_install_only_dependencies(tmpdir, mock_fetch, install_mockery):
    dep = spack.concretize.concretize_one(Spec("dependency-install"))
    root = spack.concretize.concretize_one(Spec("dependent-install"))
...
def test_install_only_dependencies_of_all_in_env(
    ...
    with ev.read("test"):
        roots = [
            spack.concretize.concretize_one(Spec("dependent-install@1.0")),
            spack.concretize.concretize_one(Spec("dependent-install@2.0")),
        ]
...
def test_cdash_configure_warning(tmpdir, mock_fetch, install_mockery, capfd):
    ...
    with capfd.disabled(), tmpdir.as_cwd():
        ...
        spec = spack.concretize.concretize_one(Spec("configure-warning"))
...
def test_install_env_with_tests_all(
    ...
    env("create", "test")
    with ev.read("test"):
        test_dep = spack.concretize.concretize_one(Spec("test-dependency"))
        add("depb")
        install("--test", "all")
