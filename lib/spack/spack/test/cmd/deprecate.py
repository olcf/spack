...
def test_deprecate_deps(mock_packages, mock_archive, mock_fetch, install_mockery):
    ...
    new_spec = spack.concretize.concretize(spack.spec.Spec("libdwarf@20130729^libelf@0.8.13"))
    old_spec = spack.concretize.concretize(spack.spec.Spec("libdwarf@20130207^libelf@0.8.10"))
...
def test_deprecate_already_deprecated(mock_packages, mock_archive, mock_fetch, install_mockery):
    ...
    deprecated_spec = spack.concretize.concretize(spack.spec.Spec("libelf@0.8.10"))

    deprecate("-y", "libelf@0.8.10", "libelf@0.8.12")

    deprecator = spack.store.STORE.db.deprecator(deprecated_spec)
    assert deprecator == spack.concretize.concretize(spack.spec.Spec("libelf@0.8.12"))
    ...
    deprecator = spack.store.STORE.db.deprecator(deprecated_spec)
    assert deprecator == spack.concretize.concretize(spack.spec.Spec("libelf@0.8.13"))
...
def test_deprecate_deprecator(mock_packages, mock_archive, mock_fetch, install_mockery):
    ...
    first_deprecated_spec = spack.concretize.concretize(spack.spec.Spec("libelf@0.8.10"))
    second_deprecated_spec = spack.concretize.concretize(spack.spec.Spec("libelf@0.8.12"))
    final_deprecator = spack.concretize.concretize(spack.spec.Spec("libelf@0.8.13"))
