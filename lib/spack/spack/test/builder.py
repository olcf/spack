...
def test_build_time_tests_are_executed_from_default_builder():
    s = spack.concretize.concretize_one(spack.spec.Spec("old-style-autotools"))
    builder = spack.builder.create(s.package)
...
def test_install_time_test_callback(tmpdir, config, mock_packages, mock_stage):
    ...
    s = spack.concretize.concretize_one(spack.spec.Spec("py-test-callback"))
    builder = spack.builder.create(s.package)
    builder.pkg.run_tests = True
...
def test_mixins_with_builders(working_env):
    ...
    """
    s = spack.concretize.concretize_one(spack.spec.Spec("builder-and-mixins"))
    builder = spack.builder.create(s.package)
