...
def test_cc_not_changed_by_modules(monkeypatch, working_env):
    s = spack.concretize.concretize_one(spack.spec.Spec("cmake"))
    pkg = s.package
...
def test_setup_dependent_package_inherited_modules(
    ...
    s = spack.concretize.concretize_one(spack.spec.Spec("cmake-client-inheritor"))
...
def test_compiler_config_modifications(
    ...
    pkg = spack.concretize.concretize_one(spack.spec.Spec("cmake")).package
...
def test_external_config_env(mock_packages, mutable_config, working_env):
    ...
    cmake_client = spack.concretize.concretize_one(spack.spec.Spec("cmake-client"))
...
def test_package_inheritance_module_setup(config, mock_packages, working_env):
    s = spack.concretize.concretize_one(spack.spec.Spec("multimodule-inheritance"))
    pkg = s.package
...
def test_wrapper_variables(
    ...
    root = spack.concretize.concretize_one(spack.spec.Spec("dt-diamond"))
...
"""
    ...
    top = spack.concretize.concretize_one(spack.spec.Spec("dt-diamond"))
...
def test_module_globals_available_at_setup_dependent_time(
    ...
    externaltool = spack.concretize.concretize_one(spack.spec.Spec("externaltest"))
    monkeypatch.setattr(
...
def test_build_system_globals_only_set_on_root_during_build(default_mock_concretization, context):
    ...
    """
    root = spack.concretize.concretize_one(spack.spec.Spec("mpileaks"))
