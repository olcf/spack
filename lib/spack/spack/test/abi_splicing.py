...
class CacheManager:
    ...
    def __enter__(self):
        self.concr_specs = [spack.concretize.concretize(Spec(s)) for s in self.req_specs]
...
def test_simple_reuse(splicing_setup):
    with CacheManager(["splice-z@1.0.0+compat"]):
        ...
        assert spack.concretize.concretize(Spec("splice-z")).satisfies(Spec("splice-z"))


def test_simple_dep_reuse(splicing_setup):
    with CacheManager(["splice-z@1.0.0+compat"]):
        spack.config.set("packages", _make_specs_non_buildable(["splice-z"]))
        assert spack.concretize.concretize(Spec("splice-h@1")).satisfies(Spec("splice-h@1"))
...
def test_splice_installed_hash(splicing_setup):
    ...
    with CacheManager(cache):
        ...
        with pytest.raises(Exception):
            spack.concretize.concretize(goal_spec)
        _enable_splicing()
        assert spack.concretize.concretize(goal_spec).satisfies(goal_spec)
...
def test_virtual_multi_splices_in(splicing_setup):
    ...
    with CacheManager(cache):
        ...
        for gs in goal_specs:
            with pytest.raises(Exception):
                spack.concretize.concretize(Spec(gs))
        _enable_splicing()
        for gs in goal_specs:
            assert spack.concretize.concretize(Spec(gs)).satisfies(gs)
...
def test_manyvariant_star_matching_variant_splice(splicing_setup):
    ...
    with CacheManager(cache):
        ...
        for goal in goal_specs:
            with pytest.raises(Exception):
                spack.concretize.concretize(goal)
        _enable_splicing()
        for goal in goal_specs:
            assert spack.concretize.concretize(goal).satisfies(goal)
...
def test_manyvariant_limited_matching(splicing_setup):
    ...
    with CacheManager(cache):
        ...
        _enable_splicing()
        for s in goal_specs:
            assert spack.concretize.concretize(s).satisfies(s)
...
def test_spliced_build_deps_only_in_build_spec(splicing_setup):
    ...
    with CacheManager(cache):
        _enable_splicing()
        concr_goal = spack.concretize.concretize(goal_spec)
