...
def create_installer(
    ...
    concretized."""
    _specs = [
        spack.concretize.concretize_one(spack.spec.Spec(s)) if isinstance(s, str) else s
        for s in specs
    ]
...
def test_get_dependent_ids(install_mockery, mock_packages):
    ...
    spec = spack.concretize.concretize_one(spack.spec.Spec("pkg-a"))
...
def test_try_install_from_binary_cache(install_mockery, mock_packages, monkeypatch):
    ...
    spec = spack.concretize.concretize_one(spack.spec.Spec("mpich"))
...
def test_check_before_phase_error(install_mockery):
    s = spack.concretize.concretize_one(spack.spec.Spec("trivial-install-test-package"))
...
def test_dump_packages_deps_ok(install_mockery, tmpdir, mock_packages):
    ...
    spec = spack.concretize.concretize_one(spack.spec.Spec(spec_name))
...
def test_dump_packages_deps_errs(install_mockery, tmpdir, monkeypatch, capsys):
    ...
    spec = spack.concretize.concretize_one(spack.spec.Spec("simple-inheritance"))
    path = str(tmpdir)
...
def test_check_deps_status_install_failure(install_mockery):
    ...
    """
    s = spack.concretize.concretize_one(spack.spec.Spec("pkg-a"))
...
def test_install_spliced(install_mockery, mock_fetch, monkeypatch, capsys, transitive):
    ...
    spec = spack.concretize.concretize_one(spack.spec.Spec("splice-t"))
    dep = spack.concretize.concretize_one(spack.spec.Spec("splice-h+foo"))
...
def test_install_splice_root_from_binary(
    ...
    original_spec = spack.concretize.concretize_one(spack.spec.Spec(root_str))
    spec_to_splice = spack.concretize.concretize_one(spack.spec.Spec("splice-h+foo"))
...
def test_setup_install_dir_grp(install_mockery, monkeypatch, capfd):
    ...
    build_task = create_build_task(
        spack.concretize.concretize_one(spack.spec.Spec("trivial-install-test-package")).package
    )
...
def test_install_fail_fast_on_detect(install_mockery, monkeypatch, capsys):
    ...
    b = spack.concretize.concretize_one(spack.spec.Spec("pkg-b"))
    c = spack.concretize.concretize_one(spack.spec.Spec("pkg-c"))
...
def test_single_external_implicit_install(install_mockery, explicit):
    ...
    s = spack.concretize.concretize_one(spack.spec.Spec(pkg))
    s.external_path = "/usr"
...
def test_overwrite_install_does_install_build_deps(install_mockery, mock_fetch):
    ...
    s = spack.concretize.concretize_one(spack.spec.Spec("dtrun3"))
...
def test_print_install_test_log_skipped(install_mockery, mock_packages, capfd, run_tests):
    ...
    s = spack.concretize.concretize_one(spack.spec.Spec(name))
    pkg = s.package
