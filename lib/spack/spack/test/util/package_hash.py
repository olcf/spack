...
def test_content_hash_all_same_but_patch_contents(mock_packages, config):
    spec1 = spack.concretize.concretize_one(Spec("hash-test1@1.1"))
    spec2 = spack.concretize.concretize_one(Spec("hash-test2@1.1"))
...
def test_content_hash_different_variants(mock_packages, config):
    spec1 = spack.concretize.concretize_one(Spec("hash-test1@1.2 +variantx"))
    spec2 = spack.concretize.concretize_one(Spec("hash-test2@1.2 ~variantx"))
    compare_hash_sans_name(True, spec1, spec2)
...
def test_content_hash_cannot_get_details_from_ast(mock_packages, config):
    ...
    failure.
    """
    spec3 = spack.concretize.concretize_one(Spec("hash-test1@1.7"))
    spec4 = spack.concretize.concretize_one(Spec("hash-test3@1.7"))
    compare_hash_sans_name(False, spec3, spec4)


def test_content_hash_all_same_but_archive_hash(mock_packages, config):
    spec1 = spack.concretize.concretize_one(Spec("hash-test1@1.3"))
    spec2 = spack.concretize.concretize_one(Spec("hash-test2@1.3"))
    compare_hash_sans_name(False, spec1, spec2)


def test_content_hash_parse_dynamic_function_call(mock_packages, config):
    spec = spack.concretize.concretize_one(Spec("hash-test4"))
