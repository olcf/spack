...
def set_up_package(name, repository, url_attr):
    ...
    s = spack.concretize.concretize(Spec(name))
    ...
    with spack.stage.Stage("spack-mirror-test") as stage:
        ...
        with spack.config.override("mirrors", mirrors):
            with spack.config.override("config:checksum", False):
                specs = [spack.concretize.concretize(Spec(x)) for x in repos]
            ...
            for name, mock_repo in repos.items():
                spec = spack.concretize.concretize(Spec(name))
                pkg = spec.package
...
def test_mirror_archive_paths_no_version(mock_packages, mock_archive):
    spec = spack.concretize.concretize(Spec("trivial-install-test-package@=nonexistingversion"))
