...
def test_git_ref_comparisons(mock_git_version_info, install_mockery, mock_packages, monkeypatch):
    ...
    spec_tag = spack.concretize.concretize_one(spack.spec.Spec("git-test-commit@git.v1.0"))
    ...
    spec_branch = spack.concretize.concretize_one(spack.spec.Spec("git-test-commit@git.1.x"))
...
def test_git_version_repo_attached_after_serialization(
    ...
    spec = spack.concretize.concretize_one(spack.spec.Spec(f"git-test-commit@{commits[-2]}"))
...
def test_resolved_git_version_is_shown_in_str(
    ...
    commit = commits[-3]
    spec = spack.concretize.concretize_one(spack.spec.Spec(f"git-test-commit@{commit}"))
