...
def test_no_version_match(pkg_name):
    spec = spack.concretize.concretize(spack.spec.Spec(pkg_name + "@2.0"))
...
def test_multimethod_calls(
    ...
    with spack.config.override(
        ...
        s = spack.concretize.concretize(spack.spec.Spec(pkg_name + constraint_str))
...
def test_target_match(pkg_name):
    ...
    for target in targets[:-1]:
        s = spack.concretize.concretize(spack.spec.Spec(pkg_name + " target=" + target.name))
        assert s.package.different_by_target() == target.name

    s = spack.concretize.concretize(spack.spec.Spec(pkg_name + " target=" + targets[-1].name))
    if len(targets) == 1:
...
def test_multimethod_calls_and_inheritance(spec_str, method_name, expected_result):
    s = spack.concretize.concretize(spack.spec.Spec(spec_str))
