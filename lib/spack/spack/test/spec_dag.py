...
def test_test_deptype(tmpdir):
    ...
    with spack.repo.use_repositories(builder.root):
        spec = spack.concretize.concretize(Spec("w"), tests=("w",))
        assert "x" in spec
...
def test_installed_deps(monkeypatch, mock_packages):
    ...
    c_spec = spack.concretize.concretize(Spec(c))
    ...
    a_spec = spack.concretize.concretize(a_spec)
...
def test_specify_preinstalled_dep(tmpdir, monkeypatch):
    ...
    with spack.repo.use_repositories(builder.root):
        b_spec = spack.concretize.concretize(Spec("pkg-b"))
...
class TestSpecDag:
    ...
    def test_preorder_node_traversal(self):
        dag = spack.concretize.concretize(Spec("mpileaks ^zmpi"))
    ...
    def test_dependents_and_dependencies_are_correct(self):
        ...
        check_links(spec)
        concrete = spack.concretize.concretize(spec)
    ...
    def test_copy_concretized(self):
        orig = spack.concretize.concretize(Spec("mpileaks"))
        copy = orig.copy()
    ...
    def test_deptype_traversal(self, spec_str, deptypes, expected):
        dag = spack.concretize.concretize(Spec(spec_str))
    ...
    def test_spec_tree_respect_deptypes(self):
        ...
        s = spack.concretize.concretize(Spec("version-test-root"))
...
def test_synthetic_construction_of_split_dependencies_from_same_package(mock_packages, config):
    ...
    root = spack.concretize.concretize(Spec("pkg-b"))
    link_run_spec = spack.concretize.concretize(Spec("pkg-c@=1.0"))
    build_spec = spack.concretize.concretize(Spec("pkg-c@=2.0"))
...
def test_synthetic_construction_bootstrapping(mock_packages, config):
    ...
    #  pkg-b@1.0
    #
    root = spack.concretize.concretize(Spec("pkg-b@=2.0"))
    bootstrap = spack.concretize.concretize(Spec("pkg-b@=1.0"))
...
def test_adding_same_deptype_with_the_same_name_raises(
    ...
    p = spack.concretize.concretize(Spec("pkg-b@=2.0"))
    c1 = spack.concretize.concretize(Spec("pkg-b@=1.0"))
    c2 = spack.concretize.concretize(Spec("pkg-b@=2.0"))
