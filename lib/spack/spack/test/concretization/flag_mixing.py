...
"""
    ...
    s1 = spack.concretize.concretize_one(Spec('y cflags="-a"'))
    assert s1.satisfies('cflags="-a -c"')


def test_mix_spec_and_dependent(concretize_scope, test_repo):
    s1 = spack.concretize.concretize_one(Spec('x ^y cflags="-a"'))
...
def test_mix_spec_and_compiler_cfg(concretize_scope, test_repo):
    ...
    s1 = spack.concretize.concretize_one(Spec('y %gcc@12.100.100 cflags="-O2"'))
...
"""
    ...
    root_spec = spack.concretize.concretize_one(Spec(spec_str))
    spec = root_spec["y"]
...
def test_two_dependents_flag_mixing(concretize_scope, test_repo):
    root_spec1 = spack.concretize.concretize_one(Spec("w~moveflaglater"))
    spec1 = root_spec1["y"]
    assert spec1.compiler_flags["cflags"] == "-d0 -d1 -d2".split()

    root_spec2 = spack.concretize.concretize_one(Spec("w+moveflaglater"))
    spec2 = root_spec2["y"]
...
def test_propagate_and_compiler_cfg(concretize_scope, test_repo):
    ...
    root_spec = spack.concretize.concretize_one(Spec("v %gcc@12.100.100 cflags=='-f1'"))
...
def test_propagate_and_pkg_dep(concretize_scope, test_repo):
    root_spec1 = spack.concretize.concretize_one(Spec("x ~activatemultiflag cflags=='-f1'"))
...
"""
    ...
    root_spec1 = spack.concretize.concretize_one(Spec("v cflags=='-f1'"))
...
"""
    ...
    root_spec2 = spack.concretize.concretize_one(Spec("v cflags=='-f1'"))
...
def test_diamond_dep_flag_mixing(concretize_scope, test_repo):
    ...
    """
    root_spec1 = spack.concretize.concretize_one(Spec("t"))
    spec1 = root_spec1["y"]
