...
def _concretize_with_reuse(*, root_str, reused_str):
    reused_spec = spack.concretize.concretize(spack.spec.Spec(reused_str))
...
def test_correct_gcc_runtime_is_injected_as_dependency(runtime_repo):
    s = spack.concretize.concretize(spack.spec.Spec("pkg-a%gcc@10.2.1 ^pkg-b%gcc@9.4.0"))
    a, b = s["pkg-a"], s["pkg-b"]
...
def test_external_nodes_do_not_have_runtimes(runtime_repo, mutable_config, tmp_path):
    ...
    s = spack.concretize.concretize(spack.spec.Spec("pkg-a%gcc@10.2.1"))
