...
def _concretize_spec_pairs(
    ...
    if len(to_concretize) == 1:
        ...
        return [concrete or spack.concretize.concretize(abstract, tests=tests)]
...
def matching_spec_from_env(spec):
    ...
    if env:
        return env.matching_spec(spec) or spack.concretize.concretize(spec)
    else:
        return spack.concretize.concretize(spec)
